...
if($_POST){
    if(empty($_POST['login']) || empty($_POST['name']) || empty($_POST['email']) || empty($_POST['password']) || $_POST['nivel'] == 3){
        ...
        if($stmt->execute()){  
            if($stmt->rowCount() > 0){ 
                echo "O login já está em uso! Tente utilizar outro.<br><br><a href='../user_register'>Voltar</a>";
            }else{
                ...
                if($stmt->execute()){  
                    if($stmt->rowCount() > 0){ 
                        ...
                        if($stmt->execute()){
                            echo "O cadastro foi realizado com sucesso!<br><br><a href='../user_register'>Voltar</a>";
                        }
                    }
                }else{
                    echo "Erro ao verificar email!<br><br><a href='../user_register'>Voltar</a>";
                }
            }
        }else{
            echo "Erro ao verificar login!<br><br><a href='../user_register'>Voltar</a>";
        }
    }
}